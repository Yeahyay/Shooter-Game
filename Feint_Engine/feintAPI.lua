...
function Feint:init(isThread)
	...
	Math = Feint.Math
	Util = Feint.Util
	Graphics = Feint.Core.Graphics
	LoveGraphics = love.graphics
	Time = Feint.Core.Time
	Log = Feint.Log
	Core = Feint.Core
	Input = Feint.Core.Input
	Debug = Feint.Core.Util.Debug
	-- luacheck: pop
	...
	World = Feint.ECS.World
	Mouse = Input.Mouse
end


-- Callbacks
function Feint.Callbacks.General.Load(arg, unfilteredArg)
...
function Feint.Callbacks.General.Update(dt)
	Time:update()
...
function Feint.Callbacks.General.Draw()
	do
...
function Feint.Callbacks.General.threaderror(thread, errorstr)
	error(string.format("Thread (%s): Error \"%s\"\n", thread, errorstr), 2)
end
-- function Feint.Callbacks.General.run()
-- end
local readyToQuit = true
function Feint.Callbacks.General.quit(index, orientation)
	if not readyToQuit then
		print("Not ready to quit")
		readyToQuit = true
		return true
	else
		print("Quitting")
...
function Feint.Callbacks.General.displayrotated(index, orientation)
end
function Feint.Callbacks.General.errorhandler(msg)
end
function Feint.Callbacks.General.lowmemory()
end


function Feint.Callbacks.Window.directorydropped(path)
end
function Feint.Callbacks.Window.filedropped(path)
end
function Feint.Callbacks.Window.focus(focus)
end
function Feint.Callbacks.Window.resize(w, h)
	Graphics:setScreenResolution(w, h)
	-- love.draw()
	-- Graphics:draw()
	uiCanvas = love.graphics.newCanvas()
	debugCanvas = love.graphics.newCanvas()--Graphics.ScreenSize.x, Graphics.ScreenSize.y)
end
function Feint.Callbacks.Window.visible(visible)
end

function Feint.Callbacks.Keyboard.Pressed(key, scancode, isrepeat)
	if key == "space" then
		print(Time:isPaused())
...
function Feint.Callbacks.Keyboard.Released(key, scancode)
end
function Feint.Callbacks.Keyboard.textedited(text, start, length)
end
function Feint.Callbacks.Keyboard.textinput(text)
end

function Feint.Callbacks.Mouse.Moved(x, y, dx, dy, istouch)
	-- print(x, y)
	Input.mousemoved(x, y, dx, dy)
end
function Feint.Callbacks.Mouse.Pressed(x, y, button, istouch, presses)
	Input.mousepressed(x, y, button, istouch)
end
function Feint.Callbacks.Mouse.Released(x, y, button, istouch, presses)
	Input.mousereleased(x, y, button, istouch)
end
function Feint.Callbacks.Mouse.wheelmoved(x, y)
end

function Feint.Callbacks.Joystick.Added(joystick)
	-- print(joystick:getGamepadMappingString())
	Input:joystickadded(joystick)
end
function Feint.Callbacks.Joystick.Removed(joystick)
end
function Feint.Callbacks.Gamepad.Axis(joystick, axis, value)
	Input:gamepadaxis(joystick, axis, value)
	-- print("gamepadaxis", joystick, axis, value)
end
function Feint.Callbacks.Gamepad.Pressed(joystick, button)
	Input:gamepadpressed(joystick, button)
	-- print("gamepadpressed", joystick, button)
end
function Feint.Callbacks.Gamepad.Released(joystick, button)
	Input:gamepadreleased(joystick, button)
	-- print("gamepadreleased", joystick, button)
end
function Feint.Callbacks.Joystick.Hat(joystick, hat, direction)
	Input:joystickhat(joystick, hat, direction)
	-- print("joystickhat", joystick, hat, direction)
end
function Feint.Callbacks.Joystick.Axis(joystick, axis, value)
	Input:joystickaxis(joystick, axis, value)
-- 	print("joystickaxis", joystick, axis, value)
end
function Feint.Callbacks.Joystick.Pressed(joystick, button)
	Input:joystickpressed(joystick, button)
	-- print("joystickpressed", joystick, button)
end
function Feint.Callbacks.Joystick.Released(joystick, button)
	Input:joystickpressed(joystick, button)
	-- print("joystickreleased", joystick, button)
end

function Feint.Callbacks.Touch.Moved(id, x, y, dx, dy, pressure)
end
function Feint.Callbacks.Touch.Pressed(id, x, y, dx, dy, pressure)
end
function Feint.Callbacks.Touch.Released(id, x, y, dx, dy, pressure)
end
