...
do
	...
	-- AssetManager:addAsset("Sprite", "PlayerSprite",
	-- 	SpriteBuilder:buildSprite(AssetManager:requestAsset("Image", "PlayerImage"), 32, 32, 11)
	-- )
end
...
DEFAULT_PHYSICS_WORLD = love.physics.newWorld(0, 9.81 * 64, true)
...
do
	for i = 1, 1 do
		...
		for i = 1, tests do
			...
			for i = 1, entities do
				local timeStart = love.timer.getTime()
				local entity, archetypeChunk = entityManager:createNewEntity{Renderer, Transform, Physics, Input}

				do
					local transform = archetypeChunk:getComponent(entity, Transform)
					transform.position = vMath.Vec3(
						love.math.random(-screenSize.x / 2, screenSize.x / 2),
						0,
						love.math.random(-screenSize.y / 2, screenSize.y / 2)
					)
					local size = love.math.random(20, 50)
					transform.size = vMath.Vec3(size, 0, size)
				end

				do
					local physics = archetypeChunk:getComponent(entity, Physics)
					physics.world = DEFAULT_PHYSICS_WORLD
					physics.type = "dynamic"
					-- physics.sensor = true,
				end

				-- local time = love.timer.getTime() - timeStart
				-- printf("signature: %s, time taken: %.9fms\n", signature, time*1000)
			end
			local time = love.timer.getTime() - timeStart
			sum = sum + time
			-- printf("created %s entities in %.9fms\n", entities, time*1000)
		end
		printf("average time: %.9fms\n", sum/tests*1000)
	end

	local function func(entity, chunk)
		local transform = chunk:getComponent(entity, Transform)

	end

	currentWorld:forEach(RenderSystem, "renderable", function(entity, chunk)
		local transform = chunk:getComponent(entity, Transform)
	...
	-- currentWorld:setComponentData(entity, Transform, function(self)
	-- 	self.position = vMath.Vec3(0, 0, 50-screenSize.y/2)
	-- 	self.size = vMath.Vec3(screenSize.x*2, 10, 100)
	-- end)
...
function load()
	...
	do
		...
		love.window.setMode(screenWidth, screenHeight, {fullscreen = false, vsync = false, resizable = true, minwidth = 32, minheight = 18, msaa = 4})
...
function gui(dt)
	...
	Slab.BeginWindow('Systems Window', {
		X = screenSize.x - screenSize.x/4 - 4;
		Y = 0;
		W = screenSize.x/4;
		H = screenSize.y/2;
		Title = "Systems View";
