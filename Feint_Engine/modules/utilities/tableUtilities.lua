...
-- function tableUtilities.INSTANCE_OF_INFO(class, name, string)
-- 	return string.format("instance of %s \"%s\" (%s)", class.Name, name, string)
-- end
--
-- function tableUtilities.BAD_ARG_ERROR(argNum, funcParameter, expectedType, recievedType)
-- 	return string.format("bad argument #%d to '%s' (%s expected, got %s)'", argNum, funcParameter, expectedType, recievedType)
-- end
-- function tableUtilities.READ_ONLY_MODIFICATION_ERROR(table, key)
-- 	return string.format("attempt to modify %s by accessing key %s", table, key)
-- end

function tableUtilities.deepCopy(object)
	local lookup_table = {}
...
function tableUtilities.deepcopy(orig)
